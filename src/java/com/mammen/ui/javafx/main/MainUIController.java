...
public class MainUIController {
    ...
    @FXML
    private TableView<SwerveWaypoint> tblWaypoints;

    @FXML
    private TableColumn<SwerveWaypoint, Double> colWaypointX, colWaypointY, colWaypointAngle, colWaypointRotation;

    @FXML
    private MenuItem mnuFileSave;

    @FXML
    private Button btnClearPoints, btnDelete;
    ...
    // Reference to the PosGraphController object that JavaFX creates when it loads
    // the fxml file.
    ...
    public void initialize() {
        ...
        Callback<TableColumn<SwerveWaypoint, Double>, TableCell<SwerveWaypoint, Double>> doubleCallback = (
                TableColumn<SwerveWaypoint, Double> param) -> {
            TextFieldTableCell<SwerveWaypoint, Double> cell = new TextFieldTableCell<>();
        ...
        EventHandler<TableColumn.CellEditEvent<SwerveWaypoint, Double>> editHandler = (
                TableColumn.CellEditEvent<SwerveWaypoint, Double> t) -> {
            SwerveWaypoint curWaypoint = t.getRowValue();

            if (t.getTableColumn() == colWaypointRotation)
                curWaypoint.setRotation(t.getNewValue());
        ...
        colWaypointRotation.setCellFactory(doubleCallback);
        ...
        colWaypointRotation.setOnEditCommit(editHandler);

        colWaypointX.setCellValueFactory(
                (TableColumn.CellDataFeatures<SwerveWaypoint, Double> d) -> new ObservableValueBase<Double>() {
                    @Override
        ...
        colWaypointY.setCellValueFactory(
                (TableColumn.CellDataFeatures<SwerveWaypoint, Double> d) -> new ObservableValueBase<Double>() {
                    @Override
                    public Double getValue() {
        ...
        colWaypointAngle.setCellValueFactory(
                (TableColumn.CellDataFeatures<SwerveWaypoint, Double> d) -> new ObservableValueBase<Double>() {
                    @Override
                    public Double getValue() {
        ...
        colWaypointRotation.setCellValueFactory(
                (TableColumn.CellDataFeatures<SwerveWaypoint, Double> d) -> new ObservableValueBase<Double>() {
                    @Override
                    public Double getValue() {
                        return Mathf.round(d.getValue().getRotation(), 2);
                    }
                });
         ...
         *************************************************/
        backend.waypointListProperty().addListener((ListChangeListener<SwerveWaypoint>) c -> {
            ...
            try {
                ...
                // Run later because we don't want to modify a list inside it's own change
                // listener.
                Platform.runLater(() -> {
        ...
        tblWaypoints.getSelectionModel().selectedIndexProperty().addListener((observable, oldValue,
                newValue) -> btnDelete.setDisable(tblWaypoints.getSelectionModel().getSelectedIndices().get(0) == -1));
    ...
    private void showSaveAsDialog() {
        ...
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Extensive Markup Language", "*.xml"));
    ...
    private void showAddPointDialog() {
        Dialog<SwerveWaypoint> waypointDialog = DialogFactory.createWaypointDialog();
