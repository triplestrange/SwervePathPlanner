...
public class SwerveTrajectory extends Trajectory {
    ...
    public static SwerveTrajectory generate(Trajectory.Config config, SwerveWaypoint[] waypoints,
        ...
        for (int i = 0; i < waypoints.length; i++) {
            double shortdist = Math.hypot(trajectory.segments[0].x - waypoints[i].getX(),
                    trajectory.segments[0].y - waypoints[i].getY());

            for (int j = 0; j < trajectory.segments.length; j++) {
                double dist = Math.hypot(trajectory.segments[j].x - waypoints[i].getX(),
                        trajectory.segments[j].y - waypoints[i].getY());
                if (dist <= shortdist) {
                    shortdist = dist;
        ...
        trajectory.gyroProfile[0] = new TrapezoidProfile(trajectory.waypoints[0].getRotation(),
                trajectory.waypoints[1].getRotation(), 0, 0, trajectory.angularVelocity, trajectory.angularAcc);
        for (int i = 1; i < waypoints.length - 1; i++) {
            ...
            double endPos = trajectory.waypoints[i + 1].getRotation();
